...
(defn run-pt2
  []
  (let [sum-to-find (bigint 1721308972)
        full-inputs (concat (:preamble puzzle9-real) (:inputs puzzle9-real))]
    (loop [current-sum-list [(first full-inputs)]
           inputs-left (rest full-inputs)
           current-sum (first full-inputs)]
      (if (= sum-to-find (+ current-sum (first inputs-left)))
        ...
        (if (< sum-to-find (+ current-sum (first inputs-left)))
          (recur (vec (rest current-sum-list)) inputs-left (- current-sum (first current-sum-list)))
